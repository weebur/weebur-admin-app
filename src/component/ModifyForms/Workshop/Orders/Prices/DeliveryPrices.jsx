...
import { productDeliveryTypes } from '../../../../../constants/product';

function DeliveryPrices({ deliveryPrices, productDelivery, name, onChange, onValueChange }) {
    const [optionNames, setOptionNames] = useState(Object.values(productDeliveryTypes));

    const { statements } = deliveryPrices;

    const updateTotal = (newStatement, index) => {
        const nextStatements = getNextStatements(deliveryPrices, newStatement, index);
                    ...
                    >
                        <PriceRow key={i}>
                            <SelectBox
                                ...
                                onChange={(_, name) => {
                                    const { price = 0 } =
                                        productDelivery.find((excursion) => excursion.type === name) || {};
                                ...
                                onSearch={(v) => {
                                    if (!v) {
                                        setOptionNames(Object.values(productDeliveryTypes));
                                        return;
                                    }
